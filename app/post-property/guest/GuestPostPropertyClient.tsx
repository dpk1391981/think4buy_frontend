...
import {
  CheckCircle, Loader2, Mail, Phone, RefreshCw, Shield,
...
export default function GuestPostPropertyPage() {
  ...
  // Email, not mobile: SMS OTP is switched off until DLT approval, so this
  // funnel verifies by email like the rest of the app.
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true); setError('');
    try {
      const { data } = await authApi.verifyEmailOtp(email.trim(), otp);
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-7 text-gray-900">
              ...
              {step === 'success' ? (
                ...
                <>
                  ...
                  <form onSubmit={handleSendOtp} className="space-y-3">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          autoComplete="email"
                          className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary-400 focus:border-transparent"
                        />
                      </div>
                    </div>
                    ...
                    >
                      {loading
                        ...
                        : <><Mail className="w-4 h-4" /> Get Code & Start Posting</>
                      }
                    </button>
                  </form>
                  ...
                  <Link
                    href="/auth?redirect=/post-property"
                ...
                <form onSubmit={handleVerifyOtp} className="space-y-4">
                  ...
                  {devOtp && (
                    <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-4 py-2.5 text-sm">
                      ...
                      <span className="text-green-700">Dev code: <strong className="text-green-800">{devOtp}</strong></span>
                    </div>
                  )}

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Enter the 6-digit code</label>
                    <input
                      type="text"
                      inputMode="numeric"
                      value={otp}
