...
import { ExternalLink, ChevronRight, MoreHorizontal, X } from 'lucide-react';
import NotificationBell from '@/components/notifications/NotificationBell';

// ── Slug → href mapping for buyer panel ────────────────────────────────────────

const BUYER_HREF: Record<string, { href: string; exact?: boolean }> = {
  dashboard:        { href: '/buyer',             exact: true },
  search_property:  { href: '/properties' },
  saved_properties: { href: '/buyer/saved' },
  my_enquiries:     { href: '/buyer/enquiries' },
  site_visits:      { href: '/buyer/site-visits' },
  messages:         { href: '/buyer/messages' },
  property_alerts:  { href: '/buyer/alerts' },
  profile:          { href: '/buyer/profile' },
  settings:         { href: '/buyer/settings' },
};

// Fallback menus rendered before DB menus load
const FALLBACK_MENUS: MenuItem[] = [
  { name: 'Dashboard',         slug: 'dashboard',        icon: 'layout-dashboard' },
  { name: 'Search Property',   slug: 'search_property',  icon: 'search' },
  { name: 'Saved Properties',  slug: 'saved_properties', icon: 'heart' },
  { name: 'My Enquiries',      slug: 'my_enquiries',     icon: 'message-square' },
  { name: 'Property Alerts',   slug: 'property_alerts',  icon: 'bell' },
...
export default function BuyerLayout({ children }: { children: React.ReactNode }) {
  ...
  const Avatar = ({ size = 9 }: { size?: number }) => (
    <div
      className="relative rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 text-white flex items-center justify-center font-bold flex-shrink-0 overflow-hidden"
      ...
      <aside className="hidden lg:flex flex-col fixed inset-y-0 left-0 w-64 bg-slate-900 z-30 shadow-2xl">
        ...
        <div className="px-5 pt-5 pb-4 border-b border-slate-800/60">
          <Link href="/" className="flex items-center gap-3 group">
            <div className="w-9 h-9 rounded-xl bg-cyan-600 group-hover:bg-cyan-500 transition-colors flex items-center justify-center shadow-lg flex-shrink-0">
            ...
            <div>
              ...
              <div className="text-slate-500 text-[11px] leading-tight">Buyer Portal</div>
            </div>
          </Link>
        </div>

        {/* User card */}
        <Link href="/buyer/profile" className="mx-3 my-3 flex items-center gap-3 p-2.5 rounded-2xl hover:bg-slate-800 transition-colors group">
          <Avatar size={9} />
        ...
        <nav className="flex-1 px-3 pb-3 overflow-y-auto">
          <div className="space-y-0.5">
            {activeMenus.map((item) => {
              ...
              const href   = BUYER_HREF[item.slug]?.href ?? '/buyer';
              return (
                <Link
                  key={item.slug}
                  href={href}
                ...
                >
                  ...
                  {active && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-cyan-300 rounded-full -ml-3" />}
                </Link>
              );
            })}
          </div>
        ...
        <div className="p-4 border-t border-slate-800/60 space-y-2">
          <Link
            href="/properties"
            className="flex items-center justify-center gap-2 w-full py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl text-sm font-bold transition-all shadow-lg"
          >
            Search Properties
          </Link>
          <Link
      ...
      <div className="hidden lg:block lg:pl-64 min-h-screen">
        <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-gray-100 px-8 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <NotificationBell panelHref="/buyer/notifications" accentClass="bg-purple-600" />
            <Link
              href="/properties"
              className="flex items-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white text-sm font-bold rounded-xl transition-all shadow-sm"
            >
              Search Properties
            </Link>
          </div>
        ...
        <header className="fixed top-0 left-0 right-0 z-30 bg-white border-b border-gray-100 safe-top"
          ...
          <div className="h-14 px-4 flex items-center justify-between">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="w-8 h-8 rounded-xl bg-cyan-600 flex items-center justify-center flex-shrink-0 shadow-md">
        ...
        <nav className="fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-gray-100 safe-bottom"
          ...
          <div className="flex items-center h-16 px-1">
            {bottomTabs.map((tab) => {
              ...
              const href   = BUYER_HREF[tab.slug]?.href ?? '/buyer';
              return (
                <Link
                  key={tab.slug}
                  href={href}
                ...
                >
                  <div className={`p-1.5 rounded-xl transition-all ${active ? 'bg-cyan-600 shadow-md' : ''}`}>
                    <Icon className={`w-[22px] h-[22px] ${active ? 'text-white' : 'text-gray-400'}`} />
                  </div>
                  <span className={`text-[10px] font-bold leading-none ${active ? 'text-cyan-600' : 'text-gray-400'}`}>
                    {tab.name.split(' ')[0]}
                  </span>
                </Link>
              );
            ...
            {moreItems.length > 0 && (
              ...
              >
                <div className={`p-1.5 rounded-xl ${moreOpen ? 'bg-cyan-600 shadow-md' : ''}`}>
                  <MoreHorizontal className={`w-[22px] h-[22px] ${moreOpen ? 'text-white' : 'text-gray-400'}`} />
                </div>
                <span className={`text-[10px] font-bold leading-none ${moreOpen ? 'text-cyan-600' : 'text-gray-400'}`}>More</span>
              </button>
            )}
          </div>
        </nav>
          ...
          <>
            ...
            <div className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-3xl overflow-hidden safe-bottom"
              ...
              <div className="px-5 pb-3 flex items-center gap-3 border-b border-gray-100">
                <div className="relative w-11 h-11 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 text-white flex items-center justify-center text-sm font-bold flex-shrink-0 overflow-hidden">
                  {user.avatar
                ...
                {moreItems.map((item) => {
                  ...
                  return (
                    <Link key={item.slug} href={href} className="flex flex-col items-center gap-2 active:scale-90 transition-transform">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm ${active ? 'bg-cyan-600 shadow-cyan-600/30 shadow-md' : 'bg-gray-100'}`}>
                        <Icon className={`w-6 h-6 ${active ? 'text-white' : 'text-gray-600'}`} />
                      </div>
                      <span className={`text-[11px] font-semibold text-center leading-tight ${active ? 'text-cyan-600' : 'text-gray-600'}`}>
                        {item.name}
                      </span>
                    </Link>
                  );
