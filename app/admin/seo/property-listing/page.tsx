    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-2xl bg-white h-full flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-lg font-bold text-gray-900">
              {editId ? 'Edit Property Listing SEO' : 'New Property Listing SEO'}
            </h2>
            <p className="text-xs text-gray-500 mt-0.5">Configure SEO for /properties-in-[city] pages</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          {tab === 'basic' && (
            <>
              <div className="grid grid-cols-2 gap-4">
                {field('City Name', 'cityName', { placeholder: 'Delhi' })}
                {field('City Slug', 'citySlug', { placeholder: 'delhi', mono: true })}
              </div>
              {field('URL Slug', 'slug', { placeholder: 'properties-in-delhi', mono: true, hint: 'auto-generated if empty' })}
              <div>
          ...
          {tab === 'seo' && (
            <>
              {field('H1 Title', 'h1Title', { placeholder: 'Properties in {city} — Buy, Sell & Rent' })}
              {field('Meta Title', 'metaTitle', { placeholder: 'Properties in {city} | Think4BuySale' })}
              {textarea('Meta Description', 'metaDescription', { placeholder: 'Find verified properties in {city}...', rows: 3 })}
              {field('Meta Keywords', 'metaKeywords', { placeholder: 'properties delhi, buy flat delhi, rent apartment delhi' })}
              {field('Canonical URL', 'canonicalUrl', { placeholder: 'https://www.think4buysale.com/properties-in-delhi', mono: true })}
              {textarea('Intro Content (Top)', 'introContent', {
                placeholder: 'HTML content displayed above the property listing...',
                rows: 5,
              })}
              {textarea('Bottom Content', 'bottomContent', {
                placeholder: 'HTML content displayed below the property listing...',
                rows: 5,
              })}
              <div>
          ...
          {tab === 'og' && (
            <>
              {field('OG Title', 'ogTitle', { placeholder: 'Properties in Delhi — Think4BuySale' })}
              {textarea('OG Description', 'ogDescription', { placeholder: 'Find verified properties...', rows: 3 })}
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function PropertyListingSeoPage() {
  const [items, setItems] = useState<PropertyCitySeo[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await seoApi.adminGetPropertyCityPages({ page, limit: 20, search: search || undefined });
      setItems(res.data.items || []);
      setTotal(res.data.total || 0);
    } catch {
      setError('Failed to load property city SEO pages');
    } finally {
      setLoading(false);
    }
  }, [page, search]);
  ...
  function openEdit(item: PropertyCitySeo) {
    setEditId(item.id);
    setForm({
  ...
  async function handleSave() {
    ...
    setError('');
    try {
      const slug = form.slug || `properties-in-${form.citySlug}`;
      const payload = { ...form, slug };
      if (editId) {
        await seoApi.adminUpdatePropertyCityPage(editId, payload);
      } else {
        await seoApi.adminCreatePropertyCityPage(payload);
      }
      setPanelOpen(false);
      load();
    } catch (e: any) {
  ...
  async function handleDelete(id: string, cityName: string) {
    ...
    try {
      await seoApi.adminDeletePropertyCityPage(id);
      load();
    } catch {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Property Listing SEO</h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage SEO content for <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs font-mono">/properties-in-[city]</code> pages
          </p>
        </div>
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr><td colSpan={5} className="py-12 text-center text-gray-400">
                No property city SEO pages yet. Click "Add City" to create one.
              </td></tr>
            ) : items.map(item => (
              <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <a href={`/properties-in-${item.citySlug}`} target="_blank" rel="noreferrer"
