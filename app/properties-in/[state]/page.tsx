...
function citySlug(c: CityEntry): string {
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const stateName = data?.name ?? params.state.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
...
export default async function PropertiesInStatePage({ params }: { params: Params }) {
  const data = await getStateData(params.state);
  if (!data) notFound();
  ...
  return (
    <>
      ...
      {/* ── Property listings filtered by state ── */}
